...
def call_gemini_for_recommendations(prompt):
    ...
    if not api_key:
        return {
            ...
            "recommendations": [
                {"aspect": "Caption Length",
                    "suggestion": "Use concise captions (under 100 chars)."},
    ...
    try:
        resp = requests.post(url, headers=headers, params={
                             "key": api_key}, json=payload, timeout=30)
...
def upload():
    ...
    text = extract_text_from_pdf(
        save_path) if ext == ".pdf" else extract_text_from_image(save_path)
...
def analyze_post():
    ...
    comments_lines = [line for line in text.splitlines(
